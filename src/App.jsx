...
export default function App() {
  ...
  const handleAddTodo = (name, author = '共通') => {
    ...
    const item = { id: 't_' + String(Date.now()) + Math.random().toString(36).slice(2, 5), name: txt, author, done: false };
                ...
                <>
                  <div className="bg-white border border-slate-200 rounded-xl p-2 shadow-xs">
                    ...
                    <div className="grid grid-cols-7 gap-1 text-center mt-1">
                      {Array.from({ length: firstDay }).map((_, i) => <div key={'e-' + i} className="min-h-[52px] bg-slate-50/40 rounded-lg" />}
                      {Array.from({ length: daysInMonth }).map((_, i) => {
                        ...
                        const cellBoxClass = 'min-h-[52px] p-0.5 rounded-lg flex flex-col items-stretch border cursor-pointer ' + (isSel ? 'border-slate-900 bg-slate-50 ring-1 ring-slate-900' : isTod ? 'border-amber-300 bg-amber-50/40' : 'border-slate-100 bg-white');
                        const dayNumClass = 'text-[11px] font-mono leading-none text-left px-1 ' + (isTod ? 'font-bold text-amber-600' : isSel ? 'font-bold text-slate-900' : 'text-slate-600');
                        return (
                          <div key={dStr} onClick={() => setSelDate(dStr)} className={cellBoxClass}>
                            <span className={dayNumClass}>{dayNum}</span>
                            <div className="flex flex-col gap-0.5 mt-0.5">
                              {evs.slice(0, 2).map(ev => (
                                <div key={ev.id} className={'text-[8px] px-1 py-0.5 rounded truncate text-left font-medium ' + (STYLES[ev.author] ? STYLES[ev.author].cell : 'bg-slate-100')}>{ev.title}</div>
                              ))}
                            </div>
                          </div>
                        );
                ...
                {recipes.filter(r => r.title.indexOf(rQuery) !== -1).map(recipe => (
                  <div key={recipe.id} className="bg-white rounded-xl border border-slate-200 shadow-xs overflow-hidden">
                    <div onClick={() => setExpRecipeId(expRecipeId === recipe.id ? null : recipe.id)} className="p-3.5 flex items-center justify-between cursor-pointer hover:bg-slate-50">
                      ...
                      <button onClick={(e) => { if (e && e.stopPropagation) e.stopPropagation(); handleDelRecipe(recipe.id); }} className="p-1.5 text-slate-300 hover:text-rose-500"><Trash2 className="w-4 h-4" /></button>
                    </div>
